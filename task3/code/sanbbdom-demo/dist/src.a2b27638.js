...
var patch = (0, _init.init)([_style.styleModule, _class.classModule, _props.propsModule, _eventlisteners.eventListenersModule]);
var vnode = null;
var sortBy = 'rank';
var nextKey = 11;
var originalMovies = [{
...
var movies = [].concat(originalMovies); // 初始渲染页面内容
...
var view = function view(movies) {
  return (0, _h.h)('div.container', [(0, _h.h)('h1.page-title', 'Top 10 movies'), (0, _h.h)('div.btn-warp', [(0, _h.h)('div', ['Sort by: ', (0, _h.h)('a.btn', {
    class: {
      active: sortBy === 'rank'
    },
    on: {
      click: [handleSortClick, 'rank']
    }
  }, 'Rank'), (0, _h.h)('a.btn', {
    class: {
      active: sortBy === 'title'
  ...
  }, 'Description')]), (0, _h.h)('a.btn.add', {
    on: {
      click: add
    }
  }, 'Add')]), (0, _h.h)('div.list', movies.map(movieView))]);
}; // 创建列表节点


var movieView = function movieView(movie) {
  return (0, _h.h)('div.row', {
    ...
    style: {
      ...
      delayed: {
        transform: "translateY(".concat(movie.top || 0, "px)"),
        opacity: '1'
      },
      remove: {
        opacity: '0',
        transform: "translateY(".concat(movie.top, "px) translateY(").concat(movie.top - 50, "px)")
      }
    },
    hook: {
  ...
  }, movie.rank), (0, _h.h)('div', {
    style: {
      width: '30%'
    }
  }, movie.title), (0, _h.h)('div', {
    style: {
      width: '65%'
    }
  }, movie.desc), (0, _h.h)('div.rm-btn', {
    on: {
      click: [remove, movie]
    }
  }, 'x')]);
...
var handleSortClick = function handleSortClick(prop) {
  sortBy = prop;
...
var add = function add() {
  var m = originalMovies[Math.floor(Math.random() * 10)];
  movies = [{
    rank: nextKey++,
    title: m.title,
    desc: m.desc
  }].concat(movies); // 第一次渲染创建 DOM，通过 movieView 中的 insert 钩子函数获取真实 DOM 的高度并记录在 movie.elmHeight 中

  render(); // 第二次渲染，设置好每行的 top 值，再渲染到页面

  render();
}; // 删除一行列表


var remove = function remove(movie) {
...
var render = function render() {
  ...
  movies = movies.reduce(function (acc, m) {
